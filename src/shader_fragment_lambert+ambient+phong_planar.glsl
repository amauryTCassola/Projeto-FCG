...
void main()
{
	...
	vec3 phong_term = Ks*light_specter*pow(max(0,dot(reflection_vector, fragment_to_camera)), phong_expoent);
	
	float opening = radians(20); //radians
	vec3 corRGB;
	
	if(!isFlashlight){
		corRGB = Kd0*light_specter*lambert + ambient_term + phong_term;
	} else{
		...
		if(cosBeta >= cosAlpha){
			...
			corRGB = ((Kd0*(1- lambert) + Kd1*lambert)*light_specter + phong_term)*spot + ambient_term*(1-spot);
			
		}
		else
			corRGB = ambient_term + phong_term;
