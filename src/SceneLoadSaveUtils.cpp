...
std::function<void(std::vector<SceneObject>&, int callerIndex, float deltaTime)> MoveFunctionMapping(std::string functionName){
...
void SaveScene(std::string filename, std::vector<SceneObject> currentScene){
    ...
    for(unsigned int i = 0; i<currentScene.size(); i++){
      ...
      //  printf("salvou %s com textura %s\n", curObj.name.c_str(), curObj.textureFilename.c_str());
...
std::vector<SceneObject> LoadScene(std::string sceneFilename){
    ...
    for(unsigned int i = 0; i<objsList.size(); i++){
        ...
        aux = ObjectLoad(objsList[i].objFilename); //gera o objeto e popula com as informações do .obj
        ...
        for(unsigned int j = 0; j<aux.size(); j++){//se for mais de um objeto, popula todos com as mesmas informações
            ...
            aux[j].textureFilenames = objsList[i].textureFilenames;

            for(unsigned int k = 0; k<aux[j].textureFilenames.size(); k++){
                aux[j].textureIds.push_back(CreateNewTexture(aux[j].textureFilenames[k], (WrapMode)aux[j].textureWrapMode));
            }

            aux[j].fragmentShaderFilename = objsList[i].fragmentShaderFilename;
            aux[j].vertexShaderFilename = objsList[i].vertexShaderFilename;

            aux[j].vertexShaderId = CreateVertexShader(aux[j].vertexShaderFilename);
            aux[j].fragmentShaderId = CreateFragmentShader(aux[j].fragmentShaderFilename);
            aux[j].gpuProgramId = CreateGPUProgram(aux[j].vertexShaderId, aux[j].fragmentShaderId);

            aux[j].velocity = glm::vec4(objsList[i].velocity[0], objsList[i].velocity[1], objsList[i].velocity[2], objsList[i].velocity[3]);
            aux[j].blockMovement = glm::vec4(objsList[i].blockMovement[0], objsList[i].blockMovement[1], objsList[i].blockMovement[2], objsList[i].blockMovement[3]);
            aux[j].decelerationRate = objsList[i].decelerationRate;

            aux[j].onMouseOverName  = objsList[i].onMouseOverName;
            aux[j].onMouseOver      = FunctionMapping(aux[j].onMouseOverName);

            aux[j].onClickName      =  objsList[i].onClickName;
            aux[j].onClick          =  FunctionMapping(aux[j].onClickName);

            aux[j].onCollisionName = objsList[i].onCollisionName;
            aux[j].onCollision     = CollisionFunctionMapping(aux[j].onCollisionName);
            ...
            aux[j].name = objsList[i].name;

            aux[j].onMoveName = objsList[i].onMoveName;
            aux[j].onMove = MoveFunctionMapping(aux[j].onMoveName);
        }
...
std::vector<SceneObject> ObjectLoad(std::string filename){
    ...
    for(size_t currentShape = 0; currentShape < newModel.shapes.size(); currentShape++){ //itera sobre cada shape que está nesse .obj
        ...
        theobject.bbox_min_min_min = bbox_min;
        theobject.bbox_max_max_max = bbox_max;
        theobject.bbox_max_max_max.w = 1.0f;
        theobject.bbox_min_min_min.w = 1.0f;

        theobject.bbox_max_max_min = glm::vec4(bbox_max.x, bbox_max.y, bbox_min.z, 1.0f);
        theobject.bbox_max_min_max = glm::vec4(bbox_max.x, bbox_min.y, bbox_max.z, 1.0f);
        theobject.bbox_max_min_min = glm::vec4(bbox_max.x, bbox_min.y, bbox_min.z, 1.0f);

        theobject.bbox_min_max_max = glm::vec4(bbox_min.x, bbox_max.y, bbox_max.z, 1.0f);
        theobject.bbox_min_max_min = glm::vec4(bbox_min.x, bbox_max.y, bbox_min.z, 1.0f);
        theobject.bbox_min_min_max = glm::vec4(bbox_min.x, bbox_min.y, bbox_max.z, 1.0f);
    ...
    for(unsigned int i = 0; i<newObjList.size(); i++){
        newObjList[i].buffers = buffers;
    }
...
void Debug_NewObjectSphere(std::vector<SceneObject>& currentScene){
    ...
    std::vector<SceneObject> aux = ObjectLoad("../../data/sphere.obj");

    currentScene.insert(currentScene.begin(), aux.begin(), aux.end());

    currentScene.back().name = "Esfera1";
    currentScene.back().textureIds.push_back(CreateNewTexture(std::string("../../data/Liberty-GreenBronze-1.bmp"), WrapMode::MIRRORED_REPEAT));
    currentScene.back().textureIds.push_back(CreateNewTexture(std::string("../../data/Liberty-Pavimentazione-1.bmp"), WrapMode::MIRRORED_REPEAT));
    currentScene.back().textureFilenames.push_back(std::string("../../data/Liberty-GreenBronze-1.bmp"));
    currentScene.back().textureFilenames.push_back(std::string("../../data/Liberty-Pavimentazione-1.bmp"));
    currentScene.back().textureWrapMode = (int)WrapMode::MIRRORED_REPEAT;
    currentScene.back().vertexShaderId = CreateVertexShader(std::string("../../src/shader_vertex.glsl"));
    currentScene.back().fragmentShaderId = CreateFragmentShader(std::string("../../src/shader_fragment.glsl"));
    currentScene.back().gpuProgramId = CreateGPUProgram(currentScene[0].vertexShaderId, currentScene[0].fragmentShaderId);
    ...
    currentScene.back().onClick = FunctionMapping(currentScene[0].onClickName);
    currentScene.back().onMouseOverName = std::string("SphereOnMouseOver");
    currentScene.back().onMouseOver = FunctionMapping(currentScene[0].onMouseOverName);
    currentScene.back().onMoveName = std::string("SphereOnMove");
    currentScene.back().onMove = MoveFunctionMapping(currentScene[0].onMoveName);
    ...
    aux = ObjectLoad("../../data/sphere.obj");

    currentScene.insert(currentScene.end(), aux.begin(), aux.end());
    ...
    currentScene.back().vertexShaderId = CreateVertexShader(std::string("../../src/shader_vertex.glsl"));
    currentScene.back().fragmentShaderId = CreateFragmentShader(std::string("../../src/shader_fragment.glsl"));
    currentScene.back().gpuProgramId = CreateGPUProgram(currentScene[1].vertexShaderId, currentScene[1].fragmentShaderId);
    currentScene.back().fragmentShaderFilename = std::string("../../src/shader_fragment.glsl");
    currentScene.back().vertexShaderFilename = std::string("../../src/shader_vertex.glsl");
    currentScene.back().active = true;
    currentScene.back().blockMovement = glm::vec4(1.0f, 1.0f, 1.0f, 0.0f);
    ...
    currentScene.back().onClick = FunctionMapping(currentScene[0].onClickName);
    currentScene.back().onMouseOverName = std::string("SphereOnMouseOver");
    currentScene.back().onMouseOver = FunctionMapping(currentScene[1].onMouseOverName);
    currentScene.back().onMoveName = std::string("SphereOnMove");
    currentScene.back().onMove = MoveFunctionMapping(currentScene[1].onMoveName);
    currentScene.back().velocity = glm::vec4(0, 0, 0, 0);
