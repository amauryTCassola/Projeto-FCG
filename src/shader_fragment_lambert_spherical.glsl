...
void main()
{
    ...
    vec3 light_specter = lightColor.rgb; //RGBA
    ...
    vec4 light_to_fragment = -lightDir; //normalize(fragment_position_world - light_position);

    // Vetor que define o sentido da câmera em relação ao ponto atual.
    vec4 camera_to_fragment = normalize(camera_position - fragment_position);
    ...
    vec3 Kd0 = texture(tex, vec2(U,V)).rgba; //refletância difusa lida da textura
	vec3 Kd1 = texture(secondaryTex, vec2(U,V)).rgba; //refletância difusa lida da textura
	float lambert = max(0, dot(fragment_normal, light_to_fragment));
	
	if(!isFlashlight){
		vec3 corRGB = Kd0*light_specter*lambert;
		...
		if(cosBeta >= cosAlpha){
			...
			vec3 corRGB = (Kd0*(1 - lambert) + Kd1*lambert)*light_specter*spot + ambient_term*(1-spot);
			color = vec4(corRGB, texture(tex, vec2(U,V)).a);
		}
		else
